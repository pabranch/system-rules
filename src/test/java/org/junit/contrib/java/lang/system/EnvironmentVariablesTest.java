...
public class EnvironmentVariablesTest {
	...
	public void environment_variable_that_is_set_to_null_in_the_test_is_null_in_the_test() {
		...
		executeTestWithRule(new Statement() {
			@Override
			public void evaluate() {
				//we need to set a value because it variable is null by default
				environmentVariables.set("dummy name", "dummy value");
	...
	@Test
	public void environment_variable_that_is_cleared_in_the_test_is_null_in_the_test() {
		final EnvironmentVariables environmentVariables = new EnvironmentVariables();
		executeTestWithRule(new Statement() {
			@Override
			public void evaluate() {
				//we need to set a value because it variable is null by default
				environmentVariables.set("dummy name", "dummy value");
				environmentVariables.clear("dummy name");
				assertThat(getenv("dummy name")).isNull();
			}
		}, environmentVariables);
	}
	...
	@Test
	public void environment_variable_that_is_cleared_in_the_test_is_not_stored_in_the_environment_variables_map() {
		final EnvironmentVariables environmentVariables = new EnvironmentVariables();
		executeTestWithRule(new Statement() {
			@Override
			public void evaluate() {
